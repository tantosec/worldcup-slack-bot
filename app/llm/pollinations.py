...
class PollinationsProvider:
    ...
    def predict_tournament_picks(self, zebra_bold, zebra_wildcard, players):
        prompt = (
            ...
            f"Rules:\n"
            f"- winner must be a real {COMPETITION_NAME} team\n"
            f"- golden_boot must be spelled exactly as in the eligible players list\n"
            f"- semi1-4 must be 4 different real {COMPETITION_NAME} teams\n"
            ...
            f"- group_goals_guess: integer, total goals across all {GROUP_STAGE_MATCH_COUNT} group stage matches\n\n"
            f"Respond ONLY with valid JSON, no other text:\n"
